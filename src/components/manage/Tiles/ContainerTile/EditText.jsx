...
class EditText extends Component {
  ...
  constructor(props) {
    ...
    if (!__SERVER__) {
      let editorState;
      if (props.data && props.data.testtest) {
        editorState = EditorState.createWithContent(
          convertFromRaw(props.data.testtest),
        );
      } else {
  ...
  onChange(editorState) {
    if (
      ...
      this.props.onChangeTile(this.props.tile, {
        ...this.props.data,
        testtest: convertToRaw(editorState.getCurrentContent()),
      });
    }
  ...
  render() {
    ...
    return (
      <div
        ...
        className={cx('tile testtest', { selected: this.props.selected })}
      ...
      >
        <Editor
          ...
          placeholder={this.props.intl.formatMessage(messages.testtest)}
          handleReturn={() => {
            if (!this.props.detached) {
              ...
              if (!includes(settings.listBlockTypes, blockType)) {
                this.props.onSelectTile(
                  this.props.onAddTile('testtest', this.props.index + 1),
                );
                return 'handled';
              }
              return 'un-handled';
        ...
        <InlineToolbar />
        {!this.props.detached &&
          (!this.props.data.testtest ||
            (this.props.data.testtest &&
              this.props.data.testtest.blocks &&
              this.props.data.testtest.blocks.length === 1 &&
              this.props.data.testtest.blocks[0].testtest === '')) && (
            <Button
              basic
              icon
